...
from xm.core.misc import ntp, logger, set_logger
from xm.core.xmnode import xmNode
from xm.services.services import start_services
...
def parse_arguments():
    ...
    parser.add_argument('--miningAddress', dest='mining_address', required=False,
                        help="xm Wallet address on which mining reward has to be credited.")
...
def main():
    ...
    logger.debug("=====================================================================================")
    logger.info("xm Path: %s", args.xm_dir)
    ...
    xmnode = xmNode(mining_address=mining_address)
    xmnode.set_chain_manager(chain_manager)

    set_logger.set_logger(args, xmnode.sync_state)

    #######
    # NOTE: Keep assigned to a variable or might get collected
    admin_service, grpc_service, mining_service, debug_service = start_services(xmnode)

    xmnode.start_listening()

    xmnode.start_pow(args.mining_thread_count)

    logger.info('xm blockchain ledger %s', config.dev.version)
    if config.user.mining_enabled:
