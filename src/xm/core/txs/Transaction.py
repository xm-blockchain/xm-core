...
from pyxmlib.pyxmlib import bin2hstr, xmHelper, XmssFast
...
from xm.core.txs import build_tx as main_build_tx
...
class Transaction(object, metaclass=ABCMeta):
    ...
    def addr_from(self):
        ...
        return bytes(xmHelper.getAddress(self.PK))
    ...
    @staticmethod
    def from_pbdata(pbdata: xm_pb2.Transaction):
    ...
    @staticmethod
    def get_slave(tx):
        addr_from_pk = bytes(xmHelper.getAddress(tx.PK))
    ...
    def set_affected_address(self, addresses_set: set):
        ...
        addresses_set.add(bytes(xmHelper.getAddress(self.PK)))
    ...
    def validate_all(self, state_container: StateContainer, check_nonce=True) -> bool:
        ...
        addr_from_pk = bytes(xmHelper.getAddress(self.PK))
    ...
    def _coinbase_filter(self):
        if config.dev.coinbase_address in [bytes(xmHelper.getAddress(self.PK)), self.master_addr]:
