...
from xm.core import config, BlockHeader
from xm.core.config import DevConfig
from xm.core.StateContainer import StateContainer
from xm.core.StateMigration import StateMigration
from xm.core.AddressState import AddressState
from xm.core.VoteStats import VoteStats
from xm.core.LastTransactions import LastTransactions
...
from xm.core.PaginatedData import PaginatedData
...
from xm.generated import xm_pb2, xmstateinfo_pb2
...
class ChainManager:
    ...
    def get_address_state(self, address: bytes) -> AddressState:
        ...
        for page in range(0, max_token_page + 1):
            ...
            for token_txn_hash in page_data:
                ...
                # is done by a xm address as an owner, which has not been
    ...
    def get_slave_pk_access_type(self, address: bytes, slave_pk: bytes) -> xm_pb2.SlaveMetadata:
        with self.lock:
    ...
    def get_token(self, address: bytes, token_txhash: bytes) -> xm_pb2.TokenBalance:
        with self.lock:
    ...
    def get_headerhashes(self, start_blocknumber):
        with self.lock:
            ...
            node_header_hash = xm_pb2.NodeHeaderHash()
    ...
    def load(self, genesis_block):
        ...
        if height == -1:
            ...
            block_number_mapping = xm_pb2.BlockNumberMapping(headerhash=genesis_block.headerhash,
    ...
    def _try_branch_add_block(self, block, dev_config: DevConfig, check_stale=True) -> bool:
        ...
        if new_block_difficulty > last_block_difficulty:
            if self._last_block.headerhash != block.prev_headerhash:
                fork_state = xmstateinfo_pb2.ForkState(initiator_headerhash=block.headerhash)
    ...
    def _rollback(self, forked_header_hash: bytes, fork_state: xmstateinfo_pb2.ForkState = None):
        """
    ...
    def add_chain(self, hash_path: list, fork_state: xmstateinfo_pb2.ForkState) -> bool:
        """
    ...
    def _fork_recovery(self, block: Block, fork_state: xmstateinfo_pb2.ForkState) -> bool:
    ...
    def _update_block_number_mapping(self, block, batch):
        block_number_mapping = xm_pb2.BlockNumberMapping(headerhash=block.headerhash,
    ...
    def update_state_container(self,
        ...
        if isinstance(tx, TransferTokenTransaction):
            ...
            if key not in state_container.lattice_pk.data:
                lattice_pk.load(key, xm_pb2.LatticePKMetadata())
    ...
    def get_block_datapoint(self, headerhash):
        ...
        data_point = xm_pb2.BlockDataPoint()
