...
from xm.core.misc import logger, db
...
class State:
    ...
    def put_fork_state(self, fork_state: xmstateinfo_pb2.ForkState, batch=None):
        self._db.put_raw(b'fork_state', fork_state.SerializeToString(), batch)

    def get_fork_state(self) -> Optional[xmstateinfo_pb2.ForkState]:
        try:
    ...
    def get_slave_pk_access_type(self, address: bytes, slave_pk: bytes) -> xm_pb2.SlaveMetadata:
        ...
        try:
            slave_metadata = xm_pb2.SlaveMetadata()
    ...
    def get_token(self, address: bytes, token_txhash: bytes) -> xm_pb2.TokenBalance:
        try:
            token_balance = xm_pb2.TokenBalance()
