...
from pyxmlib.pyxmlib import hstr2bin, mnemonic2bin, bin2hstr, xmHelper

from xm.core import config
from xm.core.AddressState import AddressState
from xm.daemon.helper import logger
from xm.daemon.helper.DaemonHelper import WalletDecryptionError, Wallet, UNRESERVED_OTS_INDEX_START
from xm.services.WalletAPIService import WalletAPIService
from xm.generated import xm_pb2, xm_pb2_grpc, xmwallet_pb2
from xm.generated.xmwallet_pb2_grpc import add_WalletAPIServicer_to_server
...
pid = os.path.join(config.user.wallet_dir, 'xm_walletd.pid')


class WalletD:
    def __init__(self):
        self._wallet_path = os.path.join(config.user.wallet_dir, 'walletd.json')
        self._public_stub = xm_pb2_grpc.PublicAPIStub(grpc.insecure_channel(config.user.public_api_server))
        self._wallet = None
        self._passphrase = None
        self.load_wallet()

    def to_plain_blocks(self, block):
        pheader = xmwallet_pb2.PlainBlockHeader()
        ...
        pblock = xmwallet_pb2.PlainBlock()
        ...
        for genesis_balance in block.genesis_balance:
            pgb = xmwallet_pb2.PlainGenesisBalance()
    ...
    def get_address_state(self, qaddress: str) -> AddressState:
        request = xm_pb2.GetAddressStateReq(address=bytes(hstr2bin(qaddress[1:])))
    ...
    def sign_and_push_transaction(self,
        ...
        push_transaction_req = xm_pb2.PushTransactionReq(transaction_signed=tx.pbdata)
        push_transaction_resp = self._public_stub.PushTransaction(push_transaction_req, timeout=CONNECTION_TIMEOUT)
        if push_transaction_resp.error_code != xm_pb2.PushTransactionResp.SUBMITTED:
    ...
    def is_slave(self, master_address: bytes, slave_pk: bytes) -> bool:
        request = xm_pb2.IsSlaveReq(master_address=master_address,
                                     slave_pk=slave_pk)
    ...
    def get_unused_ots_index(self, address: bytes, unused_ots_index_from: int) -> Optional[int]:
        request = xm_pb2.GetOTSReq(address=address,
    ...
    def relay_token_txn(self,
        ...
        for idx, qaddress in enumerate(qaddresses):
            initial_balances.append(xm_pb2.AddressAmount(address=self.qaddress_to_address(qaddress),
                                                          amount=amounts[idx]))
    ...
    def get_mini_transactions_by_address(self, qaddress: str, item_per_page: int, page_number: int) -> tuple:
        ...
        response = self._public_stub.GetMiniTransactionsByAddress(
            xm_pb2.GetMiniTransactionsByAddressReq(address=address,
    ...
    def get_transaction(self, tx_hash: str):
        ...
        response = self._public_stub.GetTransaction(xm_pb2.GetTransactionReq(tx_hash=txhash))
    ...
    def get_balance(self, qaddress: str) -> int:
        address = self.qaddress_to_address(qaddress)
        response = self._public_stub.GetBalance(xm_pb2.GetBalanceReq(address=address))
    ...
    def get_total_balance(self) -> int:
        ...
        response = self._public_stub.GetTotalBalance(xm_pb2.GetTotalBalanceReq(addresses=addresses))
        return response.balance

    def get_ots(self, qaddress: str):
        address = self.qaddress_to_address(qaddress)
        response = self._public_stub.GetOTS(xm_pb2.GetOTSReq(address=address))
        return response.ots_bitfield_by_page, response.next_unused_ots_index, response.unused_ots_index_found

    def get_height(self) -> int:
        response = self._public_stub.GetHeight(xm_pb2.GetHeightReq())
        return response.height

    def get_block(self, header_hash: str):
        headerhash = bytes(hstr2bin(header_hash))
        response = self._public_stub.GetBlock(xm_pb2.GetBlockReq(header_hash=headerhash))
        return self.to_plain_blocks(response.block)

    def get_block_by_number(self, block_number: int):
        response = self._public_stub.GetBlockByNumber(xm_pb2.GetBlockByNumberReq(block_number=block_number))
        return self.to_plain_blocks(response.block)

    def get_address_from_pk(self, pk: str) -> str:
        return self.address_to_qaddress(xmHelper.getAddress(bytes(hstr2bin(pk))))

    def get_node_info(self):
        return self._public_stub.GetNodeState(xm_pb2.GetNodeStateReq())
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='xm node')
    parser.add_argument('--xmdir', '-d', dest='xm_dir', default=config.user.xm_dir,
    ...
    parser.add_argument('--network-type', dest='network_type', choices=['mainnet', 'testnet'],
                        default='mainnet', required=False, help="Runs xm Testnet Node")
...
def main():
    ...
    config.user.xm_dir = os.path.expanduser(os.path.normpath(args.xm_dir) + xm_dir_post_fix)
    config.create_path(config.user.xm_dir, copy_files)
    config.user.load_yaml(config.user.config_path)

    daemon = Daemonize(app="xm_walletd", pid=pid, action=run)
    daemon.start()
