...
from pyxmlib.pyxmlib import mnemonic2bin, hstr2bin, bin2hstr

from xm.core import config
from xm.core.Wallet import Wallet, WalletDecryptionError
from xm.core.misc.helper import parse_hexblob, parse_qaddress
...
from xm.crypto.xmss import XMSS, hash_functions
from xm.generated import xm_pb2_grpc, xm_pb2

ENV_xm_WALLET_DIR = 'ENV_xm_WALLET_DIR'
...
class CLIContext(object):
    ...
    def get_stub_public_api(self):
        ...
        return xm_pb2_grpc.PublicAPIStub(channel)
...
def _public_get_address_balance(ctx, address):
    ...
    get_address_state_req = xm_pb2.GetAddressStateReq(address=parse_qaddress(address))
...
@click.version_option(version=config.dev.version, prog_name='xm Command Line Interface')
@click.group()
...
@click.option('--wallet_dir', default='.', help='local wallet dir', envvar=ENV_xm_WALLET_DIR)
@click.option('--json', default=False, is_flag=True, help='output in json')
@click.pass_context
def xm(ctx, verbose, host, port_pub, wallet_dir, json):
    """
...
def tx_push(ctx, txblob):
    ...
    stub = ctx.obj.get_stub_public_api()
    pushTransactionReq = xm_pb2.PushTransactionReq(transaction_signed=tx.pbdata)
...
@xm.command()
@click.option('--src', type=str, default='', prompt=True, help='signer xm address')
@click.option('--master', type=str, default='', prompt=True, help='master xm address')
@click.option('--addr_to', type=str, default='', prompt=True, help='xm Address receiving this message (optional)')
...
@xm.command()
@click.option('--src', type=str, default='', prompt=True, help='source xm address')
@click.option('--master', type=str, default='', prompt=True, help='master xm address')
...
@xm.command()
@click.option('--src', type=str, default='', prompt=True, help='signer xm address')
@click.option('--master', type=str, default='', help='master xm address')
...
@click.option('--owner', default='', prompt=True, help='Owner xm address')
...
def tx_token(ctx, src, master, symbol, name, owner, decimals, fee, ots_key_index):
    ...
    while True:
        ...
        initial_balances.append(xm_pb2.AddressAmount(address=parse_qaddress(address),
                                                      amount=amount))
...
def slave_tx_generate(ctx, src, master, number_of_slaves, access_type, fee, pk, ots_key_index):
    ...
    try:
        ...
        click.echo('Move slaves.json file from current directory to the mining node inside ~/.xm/')
    except Exception as e:
...
@xm.command()
@click.option('--owner', default='', prompt=True, help='source xm address')
@click.pass_context
def token_list(ctx, owner):
    ...
    try:
        stub = ctx.obj.get_stub_public_api()
        address_state_req = xm_pb2.GetAddressStateReq(address=owner_address)
        ...
        for token_hash in address_state_resp.state.tokens:
            get_object_req = xm_pb2.GetObjectReq(query=bytes(hstr2bin(token_hash)))
...
def state(ctx):
    ...
    """
    stub = ctx.obj.get_stub_public_api()
    nodeStateResp = stub.GetNodeState(xm_pb2.GetNodeStateReq())
