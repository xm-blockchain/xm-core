...
from xm.generated import xmstateinfo_pb2
from xm.core import config
from xm.crypto.xmss import XMSS
from xm.crypto.QRandomX import QRandomX
...
from tests.misc.helper import get_alice_xmss, get_bob_xmss, set_default_balance_size, set_hard_fork_block_number, \
    set_xm_dir, replacement_getTime, get_some_address, gen_blocks
...
class TestChainManagerReal(TestCase):
    ...
    def test_load_twice(self):
        ...
        m_fork_state = Mock(autospec=xmstateinfo_pb2.ForkState, initiator_headerhash=self.genesis_block.headerhash)
    ...
    @patch("xm.core.DifficultyTracker.DifficultyTracker.get")
    def test_add_block(self, mock_difficulty_tracker_get):
        ...
        with patch.object(DifficultyTracker, 'get', return_value=ask_difficulty_tracker('2', config.dev)):
            ...
            with patch('xm.core.misc.ntp.getTime') as time_mock:
                time_mock.return_value = 1615270948  # Very high to get an easy difficulty
...
class TestChainManager(TestCase):
    ...
    @patch('xm.core.Block.Block', autospec=True)
    def test_fork_recovery_failed(self, mock_block):
        ...
        fork_state = xmstateinfo_pb2.ForkState(
    ...
    @patch('xm.core.config')
    @patch('xm.core.ChainManager.ChainManager.re_org_limit', new_callable=PropertyMock)
