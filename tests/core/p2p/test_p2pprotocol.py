...
from xm.core.p2p import p2pPeerManager
from xm.core.p2p.p2pfactory import P2PFactory, p2p_msg_priority
...
class TestP2PProtocol(TestCase):
    ...
    def setUp(self):
        with patch('xm.core.misc.ntp.getTime', new=replacement_getTime):
        ...
        self.channel.factory._xm_node = Mock(autospec=xmNode)
        self.channel.factory._xm_node.peer_manager = Mock(autospec=p2pPeerManager)
        self.channel.factory._xm_node.peer_manager.is_banned = MagicMock(return_value=False)
        self.channel.factory._xm_node.peer_manager.trusted_addresses = MagicMock(return_value=[])
    ...
    @patch('xm.core.misc.ntp.getTime')
    ...
    @patch('xm.core.misc.ntp.getTime')
    @patch('xm.core.p2p.p2pprotocol.logger', autospec=True)
    @patch('xm.core.p2p.p2pprotocol.config.dev', autospec=True)
    def test_dataReceived_too_big(self, config_dev, logger, getTime):
        ...
        config_dev.trust_min_msxmount = 10
        getTime.return_value = 1525078652.9991353
