...
from tests.misc.helper import replacement_getTime, set_xm_dir, get_alice_xmss, get_bob_xmss
...
class TestTransactionPool(TestCase):
    ...
    @patch('xm.core.TransactionPool.TransactionPool.is_full_transaction_pool', autospec=True)
    ...
    @patch('xm.core.TransactionPool.config', autospec=True)
    ...
    def test_get_tx_index_from_pool(self):
        ...
        tx3 = make_tx(txhash=b'xmpink')

        self.txpool.add_tx_to_pool(tx1, 1, replacement_getTime())
        self.txpool.add_tx_to_pool(tx2, 1, replacement_getTime())
        self.txpool.add_tx_to_pool(tx3, 1, replacement_getTime())

        idx = self.txpool.get_tx_index_from_pool(b'xmpink')
    ...
    @patch('xm.core.TransactionPool.TransactionPool.is_full_pending_transaction_pool', autospec=True)
    ...
    @patch('xm.core.TransactionPool.logger')
    @patch('xm.core.TransactionPool.TransactionPool.is_full_pending_transaction_pool', autospec=True)
    ...
    @patch('xm.core.TransactionPool.logger')
    @patch('xm.core.txs.Transaction.Transaction.from_pbdata', return_value=make_tx())
    @patch('xm.core.TransactionPool.TransactionPool.add_tx_to_pool', return_value=True)
    ...
    @patch('xm.core.txs.Transaction.Transaction.from_pbdata', new=replacement_from_pbdata)
    ...
    @patch('xm.core.TransactionInfo.config', autospec=True)
    @patch('xm.core.TransactionPool.TransactionPool.is_full_transaction_pool', return_value=False)
