...
from tests.misc.helper import get_alice_xmss, get_bob_xmss, get_slave_xmss, replacement_getTime, set_xm_dir
...
class TestSimpleTransaction(TestCase):
    ...
    @patch('xm.core.txs.Transaction.config')
    def test_validate_tx_invalid(self, m_config, m_logger):
        ...
        with patch('xm.core.txs.TransferTransaction.TransferTransaction.amounts',
        ...
        with patch('xm.core.txs.TransferTransaction.TransferTransaction.fee', new_callable=PropertyMock) as m_fee:
            m_fee.return_value = -1
            with self.assertRaises(ValueError):
                self.tx.validate_or_raise()
        with patch('xm.core.txs.TransferTransaction.TransferTransaction.addrs_to',
        ...
        with patch('xm.core.txs.TransferTransaction.TransferTransaction.addrs_to',
            ...
            with self.assertRaises(ValueError):
                self.tx.validate_or_raise()
        with patch('xm.core.txs.TransferTransaction.TransferTransaction.addr_from',
