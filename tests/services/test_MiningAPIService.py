...
from xm.core.BlockHeader import BlockHeader
...
class TestMiningAPI(TestCase):
    def setUp(self):
        ...
        self.xmnode._p2pfactory = p2p_factory
        self.xmnode._pow = p2p_factory.pow
        ...
        self.service = MiningAPIService(self.xmnode)

    def test_GetBlockMiningCompatible(self):
        block_header = BlockHeader.create(**self.block_header_params)
        self.xmnode.get_blockheader_and_metadata = MagicMock(return_value=[block_header, BlockMetadata()])

        req = xmmining_pb2.GetBlockMiningCompatibleReq(height=10)
        ...
        # if xmNode responds with None, None, the GRPC response should be blank too
        self.xmnode.get_blockheader_and_metadata = MagicMock(return_value=[None, None])
        answer = self.service.GetBlockMiningCompatible(request=req, context=None)
    ...
    def test_GetLastBlockHeader(self):
        ...
        req = xmmining_pb2.GetLastBlockHeaderReq(height=20)
    ...
    def test_GetBlockToMine(self):
        ...
        difficulty = 100
        self.xmnode.get_block_to_mine = MagicMock(return_value=(blocktemplate_blob, difficulty))

        req = xmmining_pb2.GetBlockToMineReq(wallet_address=b'fake address')
        ...
        # What happens if xmNode could not generate a blocktemplate? Then the response should be blank.
        self.xmnode.get_block_to_mine.return_value = None
        answer = self.service.GetBlockToMine(request=req, context=None)
    ...
    def test_SubmitMinedBlock(self):
        req = xmmining_pb2.SubmitMinedBlockReq(blob=b'minedblob')

        # Submit mined block encountered an error
        self.xmnode.submit_mined_block = MagicMock(return_value=False)
        answer = self.service.SubmitMinedBlock(req, context=None)
        self.assertTrue(answer.error)

        # This time it was successful
        self.xmnode.submit_mined_block = MagicMock(return_value=True)
        answer = self.service.SubmitMinedBlock(req, context=None)
