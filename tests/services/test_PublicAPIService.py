...
from pyxmlib.pyxmlib import str2bin, hstr2bin, bin2hstr
...
from xm.core.PaginatedBitfield import PaginatedBitfield
from xm.core.Block import Block
from xm.core.TransactionMetadata import TransactionMetadata
...
from tests.misc.helper import get_alice_xmss, get_bob_xmss, replacement_getTime, set_xm_dir
...
class TestPublicAPI(TestCase):
    ...
    def test_getNodeState(self):
        ...
        xmnode.set_chain_manager(chain_manager)
        xmnode._p2pfactory = p2p_factory
        xmnode._pow = p2p_factory.pow

        service = PublicAPIService(xmnode)
        node_state = service.GetNodeState(request=xm_pb2.GetNodeStateReq, context=None)

        # self.assertEqual(__version__, node_state.info.version)  # FIXME
        self.assertEqual(xm_pb2.NodeInfo.UNSYNCED, node_state.info.state)
    ...
    def test_getKnownPeers(self):
        ...
        xmnode._p2pfactory = p2p_factory
        xmnode._pow = p2p_factory.pow
        xmnode.peer_manager = Mock()
        xmnode.peer_manager.known_peer_addresses = ['127.0.0.1', '192.168.1.1']

        service = PublicAPIService(xmnode)
        response = service.GetKnownPeers(request=xm_pb2.GetKnownPeersReq, context=None)
    ...
    def test_getStats_timeseries(self):
        with MockedBlockchain.create(10) as mock_blockchain:
            service = PublicAPIService(mock_blockchain.xmnode)
            request = xm_pb2.GetStatsReq(include_timeseries=1)
            stats = service.GetStats(request=request, context=None)
    ...
    def test_getAddressState(self):
        with set_xm_dir('no_data'):
            ...
            xmnode.set_chain_manager(chain_manager)
            xmnode._p2pfactory = p2p_factory
            xmnode._peer_addresses = ['127.0.0.1', '192.168.1.1']
    ...
    def test_getObject(self):
        ...
        with set_xm_dir('no_data'):
            ...
            Block.put_block_number_mapping(db_state,
                                           block.block_number,
                                           xm_pb2.BlockNumberMapping(headerhash=block.headerhash),
                                           None)
            context = Mock(spec=ServicerContext)
            request = xm_pb2.GetObjectReq()
    ...
    def test_getLatestData(self):
        with set_xm_dir('no_data'):
            ...
            xmnode = xmNode(mining_address=b'')
            xmnode.set_chain_manager(chain_manager)
            xmnode.get_block_from_index = MagicMock(return_value=None)
            ...
            request = xm_pb2.GetLatestDataReq(filter=xm_pb2.GetLatestDataReq.ALL,
                                               offset=0,
                                               quantity=3)
    ...
    def test_getAddressFromPK(self):
        ...
        request = xm_pb2.GetAddressFromPKReq(pk=bytes(pk))
    ...
    @patch('xm.core.config.DevConfig', new_callable=PropertyMock)
    def test_getMiniTransactionsByAddress(self, mock_dev_config):
        ...
        with set_xm_dir('no_data'):
            ...
            TransactionMetadata.update_tx_metadata(db_state, block, None)
            request = xm_pb2.GetMiniTransactionsByAddressReq(address=alice_xmss.address,
                                                              item_per_page=10,
                                                              page_number=1)
    ...
    def test_getTransaction(self):
        ...
        context = Mock(spec=ServicerContext)
        request = xm_pb2.GetTransactionReq()
    ...
    def test_getTotalBalance(self):
        with set_xm_dir('no_data'):
            ...
            context = Mock(spec=ServicerContext)
            request = xm_pb2.GetTotalBalanceReq()
    ...
    def test_getOTS(self):
        with set_xm_dir('no_data'):
            ...
            context = Mock(spec=ServicerContext)
            request = xm_pb2.GetOTSReq(address=alice_xmss.address)
    ...
    def test_getBlock(self):
        with set_xm_dir('no_data'):
            ...
            context = Mock(spec=ServicerContext)
            request = xm_pb2.GetBlockReq(header_hash=b.headerhash)
    ...
    def test_getBlockByNumber(self):
        with set_xm_dir('no_data'):
            ...
            Block.put_block_number_mapping(db_state,
                                           b.block_number,
                                           xm_pb2.BlockNumberMapping(headerhash=b.headerhash),
                                           None)
            db_state.get_block_by_number = MagicMock(return_value=b)

            context = Mock(spec=ServicerContext)
            request = xm_pb2.GetBlockByNumberReq(block_number=b.block_number)
